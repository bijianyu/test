...
class WC_Alipay extends WC_Payment_Gateway {
    ...
    function get_alipay_args( $order ) {
        ...
        /* add by charles 
        * 暂修改为 手机支付即时到账接口 如需严谨一点 可进行pc 和 mobile 的区分 
        * $is_mobile = code......// 
        * 如需同时支持pc和mobile的支付页面呈现 可对$service进行接口重定义 对 $alipay_args进行数组重组（注：pc和mobile需要传递的参数不同）
        */


        // Service parameter that decide the payment type
        if ( $this->payment_method == 'direct' ){
            // add by charles 修改即时到账接口 for mobile 
        ...
        $alipay_args = array(
            ...
            // add by charles 修改mobile支付的参数
            // "seller_email"      => $this->alipay_account, // for pc
            "seller_id"         => $this->alipay_account,  // for mobile
            // end by charles 
            "out_trade_no"      => $this->order_prefix . ltrim( $order->get_order_number(), '#' ),
            "subject"           => $subject,
            // add by charles 修改mobile支付的参数
            //"price"             => $total_fee,  // for pc
            "total_fee"         => $total_fee,    // for mobile
            // end by charles
            "quantity"          => 1,          
            "_input_charset"    => $this->charset,
            // add by charles 新增mobile支付缺少的参数
            'show_url'          => home_url(),  // for mobile
            // end by charles 
        );
        if ($this->payment_method != 'direct') {
            $add_args = array(
